...
async def send_procedure_status_notification(
    ...
    try:
        ...
        context = {
            ...
            "previous_status_text": get_status_text(previous_status),
        ...
        if new_status == 2:
            subject = f"✅ Trámite Aprobado - Folio: {procedure.folio}"
        elif new_status == 3:
            subject = f"❌ Trámite Rechazado - Folio: {procedure.folio}"
        elif new_status == 7:
            subject = f"🎉 Licencia Emitida - Folio: {procedure.folio}"
        else:
            subject = f"📋 Actualización de Estado - Folio: {procedure.folio}"
        ...
        if isinstance(result, dict) and result.get("status_code") == 202:
            ...
            message = f"Tu trámite {procedure.folio} ha cambiado de estado a: {get_status_text(new_status)}"
            if reason:
...
async def send_license_download_notification(
    ...
    try:
        ...
        subject = f"🎉 Tu Licencia Está Lista para Descarga - Folio: {procedure.folio}"
        ...
        if isinstance(result, dict) and result.get("status_code") == 202:
            ...
            # Also create in-app notification
            message = f"¡Tu licencia del trámite {procedure.folio} está lista para descarga!"
