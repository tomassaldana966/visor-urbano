...
for route_file in "$PROJECT_ROOT/apps/frontend/app/routes"/*.tsx; do
    if [[ -f "$route_file" ]]; then
        ...
        echo "**File:** \`routes/$route_name.tsx\`" >> "$DOCS_OUTPUT"
        echo "" >> "$DOCS_OUTPUT"
        
        # Search for description or comment in the file
        local description=$(head -10 "$route_file" | grep -o "//.*" | head -1 | sed 's/\/\/ *//')
        ...
        local components=$(find_related_components "$route_file")
        ...
        if [[ -n "$components" ]]; then
            while IFS= read -r component; do
                if [[ -n "$component" && "$component" != " " ]]; then
                    ...
                    local story_file=$(find "$PROJECT_ROOT/apps/frontend/app/components" -name "${component}.stories.tsx" 2>/dev/null | head -1)
                    if [[ -f "$story_file" ]]; then
                        local story_title=$(extract_story_path "$story_file")
                        local story_url_path=$(echo "$story_title" | tr '/' '-' | tr ' ' '-' | tr '[:upper:]' '[:lower:]')
        ...
        local api_calls=$(find_api_calls "$route_file")
...
for story_file in $(find "$PROJECT_ROOT/apps/frontend" -name "*.stories.tsx" | sort); do
    ...
    local exported_stories=$(grep -oE "export const [A-Za-z0-9_]*:" "$story_file" 2>/dev/null | sed 's/export const //g' | sed 's/://g')
    if [[ -n "$exported_stories" ]]; then
        echo "**Available variants:**" >> "$DOCS_OUTPUT"
        while IFS= read -r story; do
            if [[ -n "$story" && "$story" != "default" && "$story" != "Default" ]]; then
                local story_variant=$(echo "$story" | tr '[:upper:]' '[:lower:]')
...
for router_file in "$PROJECT_ROOT/apps/backend/app/routers"/*.py; do
    if [[ -f "$router_file" ]]; then
        ...
        local endpoints=$(extract_endpoints "$router_file")
        ...
        if [[ -n "$endpoints" ]]; then
            ...
            while IFS= read -r endpoint; do
                if [[ -n "$endpoint" && "$endpoint" != " " ]]; then
                    ...
                    local swagger_tag=$(echo "$router_name" | tr '_' '-')
